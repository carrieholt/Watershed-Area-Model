...
Preds <- Preds %>% add_column(yr=data$yr, CU_ID=data$stk, logR=data$logR) %>% mutate(Resid=Estimate-logR) 
ac <- Preds %>% group_by(CU_ID) %>% summarise (autocorr=acf(Resid, plot=F)$acf[2])# provides AR(1) autocorrelation
len <- Preds %>% group_by(CU_ID) %>% summarise (count=length(Resid))
...
ac <- ac %>% left_join(len) %>% left_join(unique(SRDat[, c("CU_ID", "CU_Name")])) %>% filter(abs(autocorr)>CI)


# Need to compare absolute value of ac$autcorr with len$CI, then align with stock-name and output. I think it's the Lower Thompson with AR(1) in resids
