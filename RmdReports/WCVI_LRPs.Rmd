...
```{r fig.cap = "**Figure 6**. Time-series of SMU abundances (summation of indicators stocks with average NI >0.5) relative to LRP."}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = "**Figure 7**. Time series of spawner abundances by inlet including all indicator WCVI Chinook stocks. Three reference points are included: SREP from the watershed-area model (horizontal green line) with 95% prediction intervals (green shaded area), and SMSY (horizontal yellow line) and SGEN (horizontal blue line) estimated from SREP and expert derived productivity. The reference points in this plot differ from those in Fig. 1 because all indicator stocks were included in the watershed-area model instead of only un-enhanced indicator stocks."}
yy <- Get.LRP(remove.EnhStocks=FALSE)

plotWCVI_timeseries(WCVIEsc=yy$WCVIEsc, remove.EnhStocks = FALSE)
```

```{r echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap = "**Figure 8**. Annual CU Status with enhancement, where 0 is red status and 1 is amber or green status"}
...
```
```{r , echo=FALSE, fig.cap ="**Figure 9**. Time-series of proportion of CUs within the SMU in the red zone, with enhancement"}
...
```
```{r fig.cap = "**Figure 11**. Time-series of SMU abundances (summation of all indicators stocks) relative to LRP, with all indicator stocks included."}
# Calculate distribution of overall LRPs by integrating bootstrapped LRP 
# values with uncertainty of each LRP value from TMB
LRP.samples <- rnorm(nBS*10, LRP.bs$fit, (LRP.bs$fit - LRP.bs$lwr) / 1.96)
hist(LRP.samples)
