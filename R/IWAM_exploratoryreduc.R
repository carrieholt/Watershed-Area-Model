...
# Arguments:
  ...
  # Mod: A character for the TMB model to be used. Options include:
...
# * Scale Calculation -----------------------------------------------------
  ...
  # Calculate scale for each stock as a tibble (just a modern looking df)
  digits <- SRDat %>% group_by(Stocknumber) %>% 
    summarize(maxDigits = count.dig(max(Sp)))
    # count.dig() Creates a count of the max order of spawners as digits per stock
    # count.dig() requires source loads per run - stored in environment
  SRDat <- left_join(SRDat, digits)
  SRDat <- SRDat %>% mutate(Scale = 10^(maxDigits-1))
    # creates a per stock scale
      # TOR?: Does it make sense to scale these by max digit? Are we losing
        # data resolution?


# * Calculation of Survival Covariates ------------------------------------
  # Pre-calculation of survival covariates for standard, AR(1), survival
    # If using mod=="Liermann_PriorRicSig_PriorDeltaSig" SRDat will overwrite
      # everything done below
    # Tor?: Double-check test without running below
  ...
  # stks_ar <- c("Chikamin", "Keta", "Blossom", "Situk", "Siletz", "Columbia Sp")
    # not used again - can delete
  stksNum_ar <- c(4,5,6,10,11,16) 
    # REQUIRED
    # Tor?: Test with removal
  ...
  if(mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|
    ...
    SRDat_Scale <- SRDat_Scale %>% left_join(stksOrder) %>% arrange(Stocknumber)
      ...
      # left-joins previously created scale df with stksOrder (new stock order)
      # creates a new column called ModelOrder
      # REQUIRES stksOrder obj. creation
  SRDat_Scale <- SRDat_Scale$Scale 
    # Re-writes the obj. SRDat_Scale into a vector of only the scales - now
  ...
  SRDat_surv <- SRDat %>% filter(Stocknumber %in% stksNum_surv)
    # contains only Harrison and Cowichan
  ...
  # Tor?: SRDat_ar not be run if the previous section was removed - is ind_ar required?
  ...
  # Tor? SRDat_surv is required to remove the years from Cowichan, but not
    # otherwise referenced - is this a good point to clean-up?
  ind_surv <- tibble(ind_surv= 0:(length(unique(SRDat_surv$Name))-1))
  insind_surv <- add_column(ind_surv, Name = (unique(SRDat_surv$Name)))
  Surv <- as.data.frame(read.csv("DataIn/Surv.csv"))
    # Additional Harrison and Cowichan Survival metrics
  SRDat_surv <- SRDat_surv %>% left_join(ind_surv, by="Name") %>% left_join(Surv)
    # Join New survival metrics to the existing subset df
  
  
  # Remove years 1981-1984, 1986-1987  from Cowichan (Stocknumber 23) as per 
  # Tompkins et al. 2005
  # Tor?: Is this required? Double-check that _surv is required for main model.
  SRDat_surv_Cow <- SRDat_surv %>% filter(Name == "Cowichan" & 
                                            Yr >= 1985 & Yr !=1986 &Yr != 1987)
  n_surv_Cow <- length(SRDat_surv_Cow$Yr) # length = 14
...
# * Read in watershed area data and life-history type ---------------------
  ...
  names <- SRDat %>% dplyr::select (Stocknumber, Name) %>% distinct() 
    # Why is SRDat used here instead of SRDat_std?
...
#### 2. Create data and parameter lists for TMB --------------------------------
  ...
  data$Scale <- SRDat_Scale 
    #ordered by std, AR1, surv, if all 3 Ricker models uses --> ModelOrder
