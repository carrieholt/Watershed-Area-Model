...
Bern_logistic = 0/1. Is the logistic regression Bernoulli (TRUE, 1) or Binomial (FALSE, 0). For this dummy example  on WCVI set to FALSE; usually set to TRUE
...
# Step 1.  
Box-Tidwell test to assess linearity (Assumption 1). First, data to run the logistic regression are loaded The rda file contain a list, "SMUlogisticData", and vector "CU_Names", and is saved on a "DataIn" directory.
...
```{r boxTidwell, warning=FALSE, message=FALSE, error=FALSE, echo=TRUE}
  ...
  pBoxTidwell <- All_Ests %>% filter(Param=="B_2") %>% pull(Pr...z.2..)
...
plotname <- input$plotname
  ...
  # generalized_linear_models/#:~:text=For%20type%20%3D%20%22pearson%22%20%2C,%
  # CB%86f(xi)
...
# Step 4. 
Are any residuals outliers? (i.e., are deviance residuals >3) (Assumption 3).  Note, we cannot yet evaluate influence test statistics such as Cook's distance because the hat  matrix is not provided in TMB outputs. 
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=TRUE}
  ...
  for (i in 1:n){
    ...
    # Step 10.4: Get predicted ppn of CUs above their lower benchmark for the 
    # year that was held out
    B_0 <- All_Ests %>% filter(Param=="B_0") %>% pull(Estimate)
    B_1 <- All_Ests %>% filter(Param=="B_1") %>% pull(Estimate)
