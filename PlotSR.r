...
PlotSRCurve <- function(SRDat, All_Est, SMSY_std, stksNum_ar, stksNum_surv, stks_surv, r2, removeSkagit, mod) {
  ...
  for (i in Stks){
    ...
    if(mod!="IWAM_FixedSep_RicStd" & mod!="Liermann" & mod!="Liermann_SepRicA" & mod!="Liermann_HalfNormRicVar"){
    ...
    if(mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") col.use <- "black"
    ...
    if(mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar")  polygon(x=c(smsy_ul, smsy_ll, smsy_ll, smsy_ul), y=c(-10000,-10000,max(R$Rec),max(R$Rec)), col=grey(0.8, alpha=0.4), border=NA )
...
plotWAregressionSMSY <- function (All_Est, All_Deltas, SRDat, Stream, WA,  PredlnSMSY=NA, PredlnWA, title1, mod) {
  ...
  if (mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") SMSY <- SMSY %>% left_join(Stream, by=c("Stocknumber"))
  ...
  if(mod=="IWAM_FixedSep"|mod=="IWAM_FixedSep_RicStd"|mod=="IWAM_FixedSep_Constm"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") logD1o <- All_Deltas %>% filter(Param=="logDelta1ocean") %>% select(Estimate) %>% pull() + logD1
  if(mod=="IWAM_FixedSep"|mod=="IWAM_FixedSep_RicStd"|mod=="IWAM_FixedSep_Constyi"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") {
  ...
  if(mod=="IWAM_FixedSep"|mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") {
...
plotWAregressionSREP <- function (All_Est, All_Deltas, SRDat, Stream, WA,  PredlnSREP=NA, PredlnWA, title1, mod) {
  ...
  if (mod=="IWAM_FixedSep_RicStd"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") SREP <- SREP %>% left_join(Stream, by=c("Stocknumber"))
  ...
  if(mod=="IWAM_FixedSep"|mod=="IWAM_FixedSep_RicStd"|mod=="IWAM_FixedSep_Constm"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") logN1o <- All_Deltas %>% filter(Param=="logNu1ocean") %>% select(Estimate) %>% pull() + logN1
  if(mod=="IWAM_FixedSep"|mod=="IWAM_FixedSep_RicStd"|mod=="IWAM_FixedSep_Constyi"|mod=="Liermann"|mod=="Liermann_SepRicA"|mod=="Liermann_HalfNormRicVar") {
...
plotRicA <- function (){#All_Est_Liermann, All_Est_Ricker_AllMod, All_Est_Liermann_SepRicA=NA){
  ...
  box.data <- data.frame(RicLogA=c(RicALsep$Estimate,RicALhNRV$Estimate, RicALsep_0.1$Estimate, RicALsep_0.001$Estimate,  
                                   RicALsep_uni$Estimate, RicALsep_none$Estimate, RicARstd$Estimate), 
                         Model=c(rep("Hierarchical\nbase\ncase",nRicAL), rep("Hierarchical\nhalf\nNormal",nRicAL),rep("Inv\nGamma\n(0.1, 0.1)", nRicAL), 
                                 rep("InvGamma\n(0.001, 0.001)", nRicAL), 
