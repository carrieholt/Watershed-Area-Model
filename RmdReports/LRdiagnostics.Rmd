...
3. There are no influential values (outliers). Note, we cannot evaluate this as
hat  matrix is not provided in TMB outputs.

4. There is no multicollinearity among predictors (NA, only 1 predictor)

# Steps:
1. Estimate Pearson residuals and deviance residuals (Assumption 3).
...
This code is wrapped inside a function LRdiagnostics() in a file of the same 
...
obsPpnAboveBM = Vectors of observed ppn of CUs above their lower benchmarks
  with NAs removed OR observed probability of a specified proportion of CUs
  being above their lower benchmarks
